...
@register_agent("heuristic2")
class Heuristic2(Agent):
    """
    A dummy class for your implementation. Feel free to use this class to
    add any helper functionalities needed for your agent.
    """
    def __init__(self):
        super(Heuristic2, self).__init__()
        self.name = "heuristic2Agent"
        self.dir_map = {
            "u": 0,
            "r": 1,
            "d": 2,
    ...
    @staticmethod
    def compute_heuristic(self, good_moves, chess_board, adv_pos, max_step):
        moves_h = []
        good_moves_copy = deepcopy(good_moves)
        for move in good_moves:
            # check if the next move would lead the player to be surrounded by three walls
            wall_count = 1
            for dir in range(4):
                if chess_board[move[0][0], move[0][1], dir]:
                    wall_count +=1
            if wall_count == 3: # remove the move if three walls
                good_moves_copy.remove(move)
            else:
                if wall_count == 2:
                    moves_h.append(0)
                else:
        ...
        if len(good_moves_copy) == 0: # if all moves result in cornered by three walls
            return good_moves

        # TODO: check which player is the most to the middle
        # for i in range(len(good_moves_copy)):
        #     distance_to_center = abs(move[0][0] - max_step) + abs(move[0][1] - max_step)

        #     moves_h[i] = moves_h[i] - distance_to_center

        if len(good_moves_copy) < 20:
            return good_moves_copy
        
        good_moves_copy = np.array(good_moves_copy, dtype=object)
        how_many_to_return = int(len(good_moves_copy)/2) # return the best half
        max_indexes = np.argpartition(moves_h, -how_many_to_return)[-how_many_to_return:]
        return  good_moves_copy[max_indexes].tolist()
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        best_moves = self.compute_heuristic(self, good_moves, chess_board, adv_pos, max_step)

        return self.pure_MCS_move(best_moves, chess_board, adv_pos, start_time, max_step)
    ...
    def pure_MCS_move(self, available_moves, chessboard, adv_pos, start_time, max_step):
        possible_states = []
        ...
        while time.time() - start_time < 1.9:
            if i == len(possible_states):
                i = 0
            self.random_simulations(possible_states[i], max_step)
            i += 1

        max_win_node = possible_states[0]
        max_win_rate = self.win_rate(possible_states[0])
        for i in range(len(possible_states)):
            if self.win_rate(possible_states[i]) > max_win_rate:
                max_win_node = possible_states[i]
                max_win_rate = self.win_rate(possible_states[i])
    ...
    def random_simulations(self, node, max_step):
        win_number = 0
        game_number = 2

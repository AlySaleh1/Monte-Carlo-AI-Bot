...
class MCTS_agent(Agent):
    ...
    def step(self, chess_board, my_pos, adv_pos, max_step):
        ...
        # run pure_MCS
        return self.pure_MCS_move(good_moves, chess_board, adv_pos, start_time, max_step)


    def MCTS_move(self, available_moves, chessboard, my_pos, adv_pos, start_time, max_step):
        headNode = MCTSnode(chessboard, my_pos, adv_pos, move_from_parent=None)
        most_promising_node = headNode
        Node_to_expand = self.select_node(headNode, most_promising_node)


    '''
    selection part of the MCTS
    '''
    def select_node(self, headNode, most_promising_node):
        nodes_stack = [headNode]
    ...
    def get_UCB(self, node):
        # node.parent.games_played_amount will throw an error at the root node, method not yet completed
        return self.win_rate(node) + math.sqrt(2 * (np.log(node.games_played_amount)/node.parent.games_played_amount))
    ...
    def win_rate(self, node,num, list_len=None):
        if node.games_played_amount == 0:
            print("no games played!number "+str(num)+"\t"+str(list_len))
            return 0
...
class MCTSnode():
    def __init__(self, chess_board_state, my_pos, adv_pos, parent, move_from_parent=None):
